...
(function($) {
	...
	$.fn._scrollable = function() {
		return this.map( function() {
			...
			return /webkit/i.test( navigator.userAgent ) || doc.compatMode == 'BackCompat' ? doc.body
					: doc.documentElement;
		} );
	};
							...
							.each( settings.axis.split( '' ),
									function(i, axis) {
										var Pos = axis == 'x' ? 'Left' : 'Top', pos = Pos.toLowerCase(), key = 'scroll'
												+ Pos, old = elem[key], max = $scrollTo.max( elem, axis );

										if (toff) {// jQuery / DOMElement
											attr[key] = toff[pos] + (win ? 0 : old - $elem.offset()[pos]);

											// If it's a dom element, reduce the
											// margin
											if (settings.margin) {
										...
										// Queueing axes
										if (!i && settings.queue) {
											// Don't waste time animating, if
											// there's no need.
											if (old != attr[key])
												// Intermediate animation
												animate( settings.onAfterFirst );
											// Don't animate this axis again in
											// the next iteration.
											delete attr[key];
										}
									} );
